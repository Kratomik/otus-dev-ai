...
import type { RecommendationRow } from '../types/recommendation.js';
...
export async function getActiveRecommendations(pool: Pool): Promise<{
  ...
  const result = await pool.query<RecommendationRow>(SELECT_ACTIVE_RECOMMENDATIONS);
